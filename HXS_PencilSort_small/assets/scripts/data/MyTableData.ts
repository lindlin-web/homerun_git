...
export enum ChipColor {BLUE,GREEN,RED, YELLOW, BLACK, VIOLET, CYAN};
...
export const ROWNUM:number = 5;
export const COLUMNNUM:number = 5;
...
export class MyTableData {
    ...
    public pushHoldChips(row:number, column:number,index:number) {
        let theArray = [];
        for(let i = 0; i < this.holdChips[index].length; i++) {
            let val = this.holdChips[index][i];
            theArray[i] = val;
        }
        this.tableData[row][column] = theArray;
        this.holdChips[index] = [];
    }

    public isHoldChipsEmpth() {
        let retVal:boolean = true;
        for(let i = 0; i < this.holdChips.length; i++) {
            let chips = this.holdChips[i];
            if(chips.length != 0) {
                retVal = false;
                break;
            }
    ...
    public createHoldChips() {
        ...
        let myValues = [];
        let startValue = 6.599;
        for(let i = 0; i < 3; i++) {let index = Math.floor(Math.random() * startValue);
            let random = values[index];
            startValue -= (i+1);
            values.splice(index, 1);
            myValues.push(random);
        };
        for(let i = 0; i < myValues.length; i++) {
            let myvalue = myValues[i];
            var discCount = Math.floor(Math.random() * 6) + 5;          // 5 - 10 个之间的碟码
            let createArr = this.createRandomIndexByCount(discCount,undefined,undefined,myvalue);
            
            retArr.push(createArr);
            this.holdChips[i] = [];
            for(let j = 0; j < createArr.length; j++) {
                let val = createArr[j];
