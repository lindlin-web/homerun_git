...
class channel_handler {
  async run() {
    ...
    for (let key in inject_channel_adapter) {
      ...
      if (d_channel.b_out_zip) {
        ...
        if (s_channel_name == "tiktok") {
          zip.file("config.json", '{"playable_orientation": 0}', { compression: "DEFLATE" });
        } else if (s_channel_name == "facebook") {
          let s_out_body_test;
          let s_index_data;
          s_out_body_test = s_html_content.split('<script type="text/javascript">');
          s_index_data = s_out_body_test[0];
          s_index_data += '<script type="text/javascript" src="./res.js"></script>' + "</body></html>";
          s_out_body = s_index_data;
          //
          let s_Res = null;
          s_Res = s_out_body_test[1];
          let s_remove1 = "</script>";
          let s_remove2 = "</body></html>";
          s_Res = s_Res.replace(s_remove1, "");
          s_Res = s_Res.replace(s_remove2, "");
          zip.file("res.js", s_Res, { compression: "DEFLATE" });
        }
      ...
      var dirs = s_out_dir.split("/");
      var sProjName = dirs[dirs.length - 1];
      console.log(sProjName);
      ...
      //  选择需要的类型
      if (
        s_channel_name == "liftoff" ||
        s_channel_name == "tiktok" ||
        s_channel_name == "vungle" ||
        s_channel_name == "google" ||
        s_channel_name == "moloco" ||
        s_channel_name == "applovin" ||
        s_channel_name == "ironsource" ||
        s_channel_name == "unity" ||
        s_channel_name == "facebook" ||
        s_channel_name == "mintegral"
      ) {
        let s_out_path2 = platform_1.default.join(s_out_dir, sProjName, s_out_file_name);
        platform_1.default.writeFileSync(s_out_path2, out_data);
      }
    }
  }
  _add_meta_to_meta(s_html_content, s_content) {
    if (!s_content) return s_html_content;
  ...
  _get_common_script(s_channel_name) {
    ...
    const s_pre_load_script = `window.super_pre_load_script = ${JSON.stringify(
      config_1.default.d_hot.l_pre_load_script
    )};`;
    // #### 各个版本适配文件
    const s_version_adapter_body = utils_1.default.get_json(
      config_1.default.constants.inject_version_adapter[config_1.default.version]
    );
