...
export class CoinEmitter extends Component {
  ...
  private resetParticle(particle:Node) {
      let randomx:number = Math.random()-0.5;
      let randomy:number = Math.random()-0.5;
       
      let length = Math.sqrt(randomx * randomx + randomy * randomy);
      randomx = randomx / length;
      randomy = randomy / length;

      randomx = randomx * (Math.random() * 500 + 1200);
      randomy = randomy * (Math.random() * 500 + 1200);
